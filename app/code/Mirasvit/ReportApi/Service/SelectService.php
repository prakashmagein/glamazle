...
class SelectService implements SelectServiceInterface
{
    ...
    public function createTemporaryTable(ColumnInterface $column, RequestInterface $request, TableInterface $baseTable)
    {
        ...
        $lastDimension = array_reverse($request->getDimensions())[0];
        $dimensionTable = $this->schema->getColumn($lastDimension)->getTable();

        // if the report is multidimensional we use the table of the last dimension instead of the base table
        $relationTable = $dimensionTable->getName() == $baseTable->getName()
            ? $baseTable
            : $dimensionTable;

        $select->setBaseTable($column->getTable())
            ->addFieldToSelect($relationTable->getPkField())
            ->addColumnToSelect($column, $column->getName())
            ->addFieldToGroup($relationTable->getPkField());

        foreach ($this->pills as $pill) {
            if ($pill->isApplicable($request, $column, $relationTable)) {
                $pill->take($select, $column, $relationTable, $request);
            }
        }
        ...
        $select->where($relationTable->getPkField()->toDbExpr() . '>0');
        ...
        $tmpTable->addColumn(
            $relationTable->getPkField()->getName(),
...
//                echo $insertQuery.PHP_EOL.PHP_EOL.PHP_EOL.PHP_EOL;
        ...
        $relation = $this->objectManager->create(Relation::class, [
            'leftTable'  => $table,
            'leftField'  => $table->getField($relationTable->getPkField()->getName()),
            'rightTable' => $relationTable,
            'rightField' => $relationTable->getPkField(),
            'type'       => '11',
        ]);
